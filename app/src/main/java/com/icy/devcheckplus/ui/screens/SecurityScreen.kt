...
import com.icy.devcheckplus.data.SecurityDataProvider
...
fun SecurityScreen(
    ...
    LaunchedEffect(Unit) {
        sections = SecurityDataProvider.getSecuritySections(context)
        loading = false
    }
    ...
    if (loading) {
        ...
        if (filtered.isEmpty()) {
            ...
            LazyColumn(state = listState, modifier = modifier.fillMaxSize()) {
                item(key = "header") { GlassSectionHeader(title = "SECURITY", icon = Icons.Default.Security) }
                items(filtered, key = { it.title }) { sec -> InfoSectionCard(section = sec, category = PinnableCategory.SOFTWARE) }

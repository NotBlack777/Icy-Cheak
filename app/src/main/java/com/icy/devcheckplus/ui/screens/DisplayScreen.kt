...
import com.icy.devcheckplus.data.DisplayDataProvider
...
fun DisplayScreen(
    ...
    LaunchedEffect(Unit) {
        sections = DisplayDataProvider.getDisplaySections(context)
        loading = false
    }
    ...
    if (loading) {
        ...
        if (filtered.isEmpty()) {
            GlassEmptyState(icon = Icons.Default.SearchOff, title = "Nothing matches \"$searchQuery\"", message = "Try different search term.")
        } else {
            ...
            LazyColumn(state = listState, modifier = modifier.fillMaxSize()) {
                item(key = "header") { GlassSectionHeader(title = "DISPLAY", icon = Icons.Default.DisplaySettings) }

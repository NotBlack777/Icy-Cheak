...
import com.icy.devcheckplus.data.CodecDataProvider
...
fun CodecScreen(
    ...
    LaunchedEffect(Unit) {
        sections = CodecDataProvider.getCodecSections()
        loading = false
    }
    ...
    if (loading) {
        ...
        if (filtered.isEmpty()) {
            ...
            LazyColumn(state = listState, modifier = modifier.fillMaxSize()) {
                item(key = "header") { GlassSectionHeader(title = "MEDIA CODECS", icon = Icons.Default.Movie) }
